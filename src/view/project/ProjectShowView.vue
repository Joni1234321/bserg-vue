<template>
	<div class="show">
		...
		<div class="main">
			<div v-for="(section,i) in project.details?.sections" class="section">
				<p class="section-title">{{ section.title }}</p>
				<div v-for="(field, fieldIndex) in section.fields" :style="{borderLeftColor: getBorderColor(field.type)}"
				     class="section-field">
					<button class="remove-button" @click="removeField(i, fieldIndex)"> x</button>
					<p> {{ field.value }}</p>
				</div>
				<div v-if="edit[i]" class="section-field">
					<input v-model="editFields[i]" class="add-field" @keyup.esc="edit[i] = false"
